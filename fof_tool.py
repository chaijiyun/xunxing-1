...
def check_password():
    ...
    if not st.session_state["password_correct"]:
        ...
        with col2:
            pwd_input = st.text_input(label="", type="password", key="password_input")
...
if check_password():
    ...
    if uploaded_file:
        ...
        # 3. 功能标签页 (合并后的重构布局)
        # ==========================================
        tabs = st.tabs(["🚀 寻星配置组合全景图", "🔍 寻星配置穿透归因分析", "⚔️ 配置池产品分析"])
        ...
        with tabs[1]:
            if sel_funds:
                ...
                # 第一层：权重与风险来源
                st.markdown("#### 🧩 第一阶段：资产分配与风险归因")
                cw1, cw2 = st.columns(2)
                with cw1:
                    st.plotly_chart(px.pie(names=list(weights.keys()), values=list(weights.values()), hole=0.4, title="资金权重分布 (Capital)"), use_container_width=True)
                with cw2:
                ...
                # 第二层：成分性格与走势
                st.markdown("---")
                st.markdown("#### 📊 第二阶段：成分性格与收益特征")
                df_sub = df_db[sel_funds].dropna()
                st.plotly_chart(px.line(df_sub.div(df_sub.iloc[0]), title="底层成分净值走势对比"), use_container_width=True)
                
                c_char1, c_char2 = st.columns([1, 1])
                with c_char1:
                ...
                with c_char2:
                    char_data = [{"产品": f, "上行捕获": calculate_metrics(df_sub[f], bench_sync_raw)['上行捕获'], 
                                 "下行捕获": calculate_metrics(df_sub[f], bench_sync_raw)['下下行捕获'], 
                ...
                final_table = []
                for f in sel_funds:
                    m_f = calculate_metrics(df_sub[f], bench_sync_raw)
                    final_table.append({
                        "产品名称": f, "资金权重": weights[f], "风险贡献占比": risk_pct.get(f, 0),
                        "年化收益": m_f['年化收益'], "最大回撤": m_f['最大回撤'], "下行捕获": m_f['下行捕获']
                    })
                st.table(pd.DataFrame(final_table).set_index("产品名称").style.format("{:.2%}"))
            else:
                st.info("👈 请先在左侧挑选组合成分。")
