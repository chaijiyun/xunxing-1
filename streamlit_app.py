...
# ==========================================
# 寻星配置分析系统 v6.5.2 (Production Candidate)
# Author: 寻星架构师
# Context: Web全栈 / 量化金融 / 极度求真
# Update: 
#   1. 完整展开代码格式，恢复至标准工程规范
#   2. 包含 v6.5.1 的所有核心修复 (非同步归因、捕获率格式统一)
#   3. 增强代码注释与可读性
# ==========================================
...
def check_password():
    ...
    if not st.session_state["password_correct"]:
        ...
        st.markdown("<h1 style='text-align: center; color: #1E40AF;'>寻星配置分析系统 v6.5.2 <small>(Prod)</small></h1>", unsafe_allow_html=True)
    ...
    if uploaded_file:
        ...
        with st.sidebar.expander("⚙️ 寻星配置参数", expanded=False):
            ...
            edited_master = st.data_editor(
                ...
                use_container_width=True, hide_index=True, key="master_editor_v652"
            )
        ...
        if sel_funds and not df_db.empty:
            ...
            if not df_port.empty:
                ...
                if fee_mode_label != "组合策略表现":
                    ...
                    for f in sel_funds:
                        # [Core Fix] 单独提取并清洗每个基金的数据，防止因为起始日不同被丢弃
